# medallion-lab Specification

## Purpose

Govern the Medallion architecture study-case pipeline that builds three layered SQLite databases (bronze → silver → gold) from the read-only MarketHub ecommerce dataset, mapping the Data Engineering roadmap's star-schema and layered-pipeline content into a reproducible, stdlib-only capability.
...
- The `medallion-lab` capability spec.
...
- Slowly Changing Dimensions (SCD), partitioning, and streaming ingestion (not covered by the star-schema snapshot design).

## Related Code

- `script/02-python/medallion_pipeline.py` — orchestrator implementation
- `script/01-sql/medallion/01-bronze.sql`, `02-silver.sql`, `03-gold.sql` — layer SQL scripts
- `data/medallion/` — generated layer databases
- `learning/05-dwh-architecture/README.md` — DWH module home
- `learning/05-dwh-architecture/medallion-case.md` — mapping + lineage doc
