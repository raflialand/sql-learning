...
- **Silver (`_silver.sql`):** (1) all six DQ dimensions evaluated, each applied or N/A-with-reason; (2) no N/A without a reason; (3) applied subset covers every dataset quirk; (4) row counts preserved (conform + flag, never drop); (5) SQL runs without error.
