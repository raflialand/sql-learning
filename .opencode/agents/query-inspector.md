---
description: Inspects learner-submitted SQL queries for query-logic correctness and business-requirement alignment; writes query-analysis.md to docs/03-query-inspector/
mode: subagent
---
# query-inspector
...
- A learner-referenced `.sql` or `.txt` file located under `script/01-sql/` (the required input; analysis does not begin without it)

## Outputs

- `docs/03-query-inspector/query-analysis.md` — the full analysis report. If this file already exists, write a dated variant (e.g. `query-analysis-<YYYY-MM-DD>.md`) instead of silently overwriting.
...
6. Write `docs/03-query-inspector/query-analysis.md` (or a dated variant if it already exists), including the analyzed query, findings, mismatches, and recommended queries.
